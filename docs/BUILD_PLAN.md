...
| P6 | Vercel Hobby **cron limits** — *job count no longer binds; daily granularity still does* | Re-read 2026-08-28: Hobby now allows **100 cron jobs per project** (the 2-per-team cap was removed 2026-01-20), but still caps frequency at **once per day** with **per-hour precision** (a `0 22 * * *` job fires anywhere in 22:00–22:59). So a second daily job is fine and a sub-daily one fails at deploy. Two jobs today, spaced two hours apart so their slack cannot overlap: precompute (2.3) at 20:00 UTC, the §10 regression replay at 22:00 UTC. `lib/db/ai-regression-run.test.ts` asserts the once-per-day and spacing properties against `vercel.json`. |
