...
--
-- N5 (docs/NHFR_2026_PLAN.md §Deferred) added the three NHFR relations here on the same
-- idempotent-file principle: fact_nhfr_facility, agg_nhfr_counts and agg_nhfr_by_type were loaded
-- live by plan N1/N2 with no registry entry, so queryDataset refused all three outright.
--
...
insert into dataset_registry (
  ...
  ('district_representative',
   ...
   'docs/LEGISLATIVE_DISTRICTS.md'),
  -- D2.6. Deferred by D1.6 on two grounds, both now answered: the table had no settled semantics
  -- until D2.3-D2.5 gave it a form, a review lifecycle and a public ledger, and it carries
  -- submitter_email. queryDataset reads with the service role, so RLS protects nothing here and
  -- the is_queryable flags below ARE the access control: submitter_email, reviewed_by and
  -- session_id are false, which is exactly the set the D2.5 ledger refuses to publish. Exposure
  -- is public because the queryable remainder is what /districts/corrections shows anyone.
  ('district_correction',
   ...
   'docs/LEGISLATIVE_DISTRICTS.md'),
  -- D3.4 §1 (docs/LEGISLATIVE_DISTRICTS_PLAN.md §6). agg_bhw_by_district (D3.1) reaches the
  -- generic queryDataset tool with no new tool code, which is the whole point of the registry.
  ('agg_bhw_by_district',
   ...
   'docs/LEGISLATIVE_DISTRICTS.md'),
  -- N5 (docs/NHFR_2026_PLAN.md §Deferred): the three NHFR relations, unqueryable until now.
  -- exposure is public because all three are already anon-readable over PostgREST - this
  -- dictionary does not change what anyone can read, only what queryDataset is willing to.
  ('fact_nhfr_facility',
...
from (values
  ...
  -- district_correction, added by D2.6. The three false flags at the end of session_id,
  -- submitter_email and reviewed_by are load-bearing: see the registry row note above.
  ...
  -- agg_bhw_by_district, added by D3.4 §1
  ...
  -- fact_nhfr_facility, agg_nhfr_counts, agg_nhfr_by_type, added by N5 (docs/NHFR_2026_PLAN.md)
